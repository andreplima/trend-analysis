...
IF %flag_preprocess%=="1" (
  python -m pdb preprocess.py ..\Configs\%1\%2\preprocess_%1_%2.cfg
)

IF %flag_forecast%=="1" (
  python -m pdb forecast.py   ..\Configs\%1\%2\forecast_%1_%2.cfg
)

IF %flag_optimise%=="1" (
  python -m pdb optimise.py   ..\Configs\%1\%2\optimise_%1_%2.cfg
)

IF %flag_measure%=="1" (
  python -m pdb measure.py    ..\Configs\%1\%2\measure_%1_%2.cfg
)
